...
Spring的JdbcTemplate和Apache的DBUtils并不是框架，而是其于jdbc的简单封装。
...
注意事项:  
1. Dao文件一般是Java bean，一般命名为XxxDao.java
2. mysql全局配置文件，一般直接放在Resource下
3. 映射文件一般和dao类名一致，即XxxDao.xml或XxxMapper.xml
4. mybatis的映射文件位置必须和dao接口的包结构相同，即：
java src中：`cn.com.daliu.XxxDao.java`
对应Resource下：`cn/com/daliu/XxxDao.xml(或XxxMapper.xml)`
...
<!--namespace对应的是java dao类-->
<mapper namespace="cn.com.dao.IUserDao">
    <!--配置查询所有-->
    <!--id是对应的java dao方法的名称-->
    <!--即对应：cn.com.dao.IUserDao.java的findAll()方法-->
    <!--cn.com.dao.IUserDao.xml对应cn.com.dao.User 包路径必须严格相同, 都为cn.com.dao-->
